...
public class CurrentAttendanceStateForProfessorActivity extends ActionBarActivity implements IOCallback {

    private CurrentAttendanceStateForProfessorAdapter currentAttendanceStateForProfessorAdapter;
    ...
    private ArrayList<CurrentAttendanceStateForProfessorBean> currentAttendanceStatePiece = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.professor_attendancebook);

        aQuery = new AQuery(this);
        id = getIntent().getStringExtra("id");

        listView = (ListView)findViewById(R.id.professor_listview);
        currentAttendanceStateForProfessorAdapter = new CurrentAttendanceStateForProfessorAdapter(this);
                    ...
                    try {
                        ...
                        handler.post(new Runnable() {
                            ...
                            public void run() {
                                ...
                                Log.i("현재 강의하는 수업", lectureName.toString());

                                mySocketIO.emit("requestCurrentProfessorAttendanceState", new IOAcknowledge() {
                                    @Override
                                    public void ack(Object... objects) {
                                        try {
...
//                            currentAttendanceStateForStudentAdapter.add(new CurrentAttendanceStateForStudentBean(stat, lname, pname));
                                                currentAttendanceStatePiece.add(new CurrentAttendanceStateForProfessorBean(stat, lname));
                                                Log.i("출결현황", "현황:" + stat + ", 강의명:" + lname);
                                            }
                                            ...
                                            for (int i = 0; i < lectureName.toArray().length; i++) {
                                                ...
                                                currentAttendanceStateForProfessorAdapter.add(new CurrentAttendanceStateForProfessorBean(lectureName.toArray()[i].toString(), attendance, lateness, absence));
                                            }
                                        } catch (JSONException e) {
                                            e.printStackTrace();
                                        }
                                        ...
                                        handler.post(new Runnable() {
                                            @Override
                                            public void run() {
                                                listView.setAdapter(currentAttendanceStateForProfessorAdapter);
                                            }
                                        });
                                    }
                                }, id);
